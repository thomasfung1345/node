...
app.post('/api/create', function(req, res) {
	MongoClient.connect(mongourl, function(err, db) {
		...
		db.collection('account').findOne({acc:request.acc, pwd:request.pwd}, function(err, user) {
			...
			uploadPhoto(db,"",request.borough,request.building,request.cuisine,request.lat,request.lon,request.name,request.street,request.acc,request.zipcode, function(result) {
